...
import { Check, Copy, Laptop, Link2, ShieldCheck, TerminalSquare, Trash2 } from "lucide-react";
...
export default function CliControl() {
  ...
  const loadDevices = () => fetch("/api/cli/devices", { credentials: "include" })
    .then((res) => res.ok ? res.json() : Promise.reject(new Error("Could not load devices")))
    .then(setDevices)
    .catch(() => setDevices([]));
  ...
  const approve = async () => {
    ...
    try {
      ...
      setMessage(`${body.device.deviceName} approved. Return to the terminal.`);
      setCode("");
      window.setTimeout(loadDevices, 3500);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Could not link device");
    } finally {
      setBusy(false);
    }
  };
  ...
  const revoke = async (id: string) => {
    await fetch(`/api/cli/devices/${encodeURIComponent(id)}`, { method: "DELETE", credentials: "include" });
    await loadDevices();
  };
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <Card className="overflow-hidden border-signal/25">
            ...
            <CardContent className="space-y-4 p-5">
              ...
              {message && <p className="text-center font-mono text-xs text-signal">{message}</p>}
            </CardContent>
          </Card>
              ...
              {devices.map((device) => (
                <div key={device.id} className="flex flex-wrap items-center justify-between gap-4 px-4 py-3.5">
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 font-mono text-[10px] text-muted-foreground">{device.platform || "unknown platform"} · linked {new Date(device.createdAt).toLocaleDateString()} · {device.lastUsedAt ? `last used ${new Date(device.lastUsedAt).toLocaleString()}` : "never used"}</div>
                  </div>
                  {!device.revokedAt && <Button variant="destructive" size="sm" onClick={() => revoke(device.id)}><Trash2 /> Revoke</Button>}
                </div>
              ))}
            </div>
