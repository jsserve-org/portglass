...
export type SharePageProps = {
  token: string;
  /** Server-resolved meta + (unlocked) snapshot — skips the client fetch. */
  initial?: Meta;
  /** Server-resolved terminal state (not found / revoked / expired). */
  initialError?: string;
};
...
export default function SharedReport({ token, initial, initialError }: SharePageProps) {
  const [meta, setMeta] = useState<Meta | null>(initial ?? null);
  const [data, setData] = useState<Snap | null>(initial?.data ?? null);
    ...
    <div className="share-page">
      <div className="share-report">
        ...
        <div className="share-meta">
          {data.kind === "scan" ? (
            <>
              ...
              <span><Clock size={12} /> Started {fmtDateTime(data.run.startedAt)}</span>
            </>
          ) : (
          ...
          <span className="share-captured"><Clock size={12} /> Snapshot {fmtDateTime(data.capturedAt)}</span>
        </div>

        <div className="share-findings">
          <table className="findings-table table-share">
            <thead>
              <tr>
        ...
        <footer className="share-foot">
          ...
          {meta?.expiresAt && <> · expires {fmtDate(meta.expiresAt)}</>}
        </footer>
      </div>
    </div>
  );
